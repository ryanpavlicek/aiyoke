...
test("deadline expiry and caller cancellation remain distinct", async () => {
  ...
  if (timedOut.kind === "failure") assert.equal(timedOut.failure.kind, "timeout");

  const cancellation = new AbortController();
  cancellation.abort();
  const cancelled = await runtime.execute(request, { signal: cancellation.signal });
  assert.equal(cancelled.kind, "failure");
  if (cancelled.kind === "failure") assert.equal(cancelled.failure.kind, "cancelled");
});
...
test("survives deterministic policy pressure without exceeding resource bounds", async () => {
  ...
  const events: RuntimeEvent[] = [];
  const runtime = new HarnessRuntime(
    { ...options, maxConcurrency: 4, maxBatchSize: 32 },
    {
      adapters: new AdapterRegistry().register("primary", {
        async invoke() {
          calls += 1;
  ...
  const budgetRuntime = new HarnessRuntime(
    { ...options, maxConcurrency: 4, maxBatchSize: 32, maxEstimatedCostUsd: 0.0001 },
    {
      adapters: new AdapterRegistry().register("primary", {
        async invoke() {
          return {
  ...
  assert.equal(
    budgetResults.every(
      (result) => result.kind === "failure" && result.failure.kind === "budget-exhausted"
    ),
    true
  );
