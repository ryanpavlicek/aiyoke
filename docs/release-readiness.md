...
| Supported target rendering | Partial | Strict artifact-contract fixtures cover every supported target, environment-only credentials, native hook/MCP/plugin/marketplace shapes, and secret canaries. Exact Claude Code and Codex npm CLIs plus a SHA-256-pinned Grok Build Windows binary pass locally: Claude parses the MCP entry, Codex validates and discovers the ChatGPT marketplace, and Grok `inspect --json` discovers instructions, skills, and the MCP endpoint. The pinned Linux CI execution is pending. Provider targets expose tested OpenRouter and xAI endpoints and credential ports. |
