...
| CI | Proven | Run `29532772411` passed separate static, six-platform Node 22/24 test/build/isolation, coverage, exact npm-package install/import/CLI, production audit, native-runtime, and framework-runtime jobs. Dependency review is configured for pull requests and intentionally skipped on pushes. |
...
| Public documentation | Proven | README covers product boundary, installation, five-minute setup, concepts, support, workflow, configuration, commands, recovery, providers, extensions, architecture, troubleshooting, development, and security. Supporting docs provide complete CLI/API references, compatibility, configuration/migrations, extension signing/isolation, runtime acceptance, release/rollback operations, examples, and a security threat model. |
| Production runtime templates | Partial | All five registered templates generate a standard-library execution facade with adapter/guard registries, typed lifecycle state, deadlines/cancellation, retry/fallback/circuit breaking, validation/repair, redacted events, approval, cache/evaluation ports, budgets, and bounded batch concurrency. Terminal policy failures cannot fall through to fallback routes. Cache hits, misses, stores, and failures are observable; cache/evaluation failures degrade safely while guard/approval failures fail closed. Registered tooling and evaluation modules provide guarded execution, reproducible suites, report/feedback ports, and regression decisions. Native suites pass Node, TypeScript, Python, Go, and Rust compiler/formatter gates, including the boundary tests in CI run `29536646599`. Framework adapters compile and execute against pinned real dependencies. Registered OpenRouter/xAI Responses adapters include native malformed/oversized-response tests in every language; Rust uses the documented typed transport port. An explicit opt-in command exercises a generated adapter against OpenRouter without exposing credentials or outputs. A closed validator now proves all seven machine-readable family entries, implementation/port delivery variants, safe existing templates, and acceptance artifacts executed by the native job; hosted execution of the final timeout/cancellation and matrix tranche remains open. |
